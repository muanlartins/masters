...
não usar \texttt{clean\_df} (\S{}\ref{sec:discussao_clean}) e a
contagem das ``22 \emph{features}'' do \emph{combined}. As demais
limitações (métrica F1 ancorada mas não publicada como chamada exata,
semente e hiperparâmetros não publicados, substituição do SVM no
\emph{combined}, cobertura reduzida do \emph{sweep} combinado e alcance
da generalização) ficam agrupadas ao final.
...
\paragraph{Métrica F1: ancorada no paper, confirmada empiricamente.}
...
\paragraph{Semente e hiperparâmetros não publicados pelo paper.}
...
\paragraph{Substituição do SVM RBF no \emph{combined}.}
...
\paragraph{Cobertura limitada do \emph{sweep} combined.}
