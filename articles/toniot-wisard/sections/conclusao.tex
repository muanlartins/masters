...
estruturadas. É esse \emph{footprint} reduzido --- e a inferência em
...
porém, é a explicitação dos três problemas estruturais dos CSVs
públicos do TON\_IoT (\emph{leak} categórico, \emph{leak} temporal
universal, divergência de tamanho contra o paper original) --- o que
esperamos sirva de referência para trabalhos futuros sobre esse
dataset.

\paragraph{Disponibilidade de código e dados.} Todo o pipeline ---
\emph{notebook} de referência (\texttt{06\_consolidado\_grupos.ipynb}),
\emph{scripts} de \emph{sweep} e \emph{benchmark}, e os JSONL/JSON de
...
A biblioteca \texttt{wisardpkg} (com o termômetro logarítmico e a
métrica \texttt{deployedSizeBytes}) é fixada por \emph{commit} em
