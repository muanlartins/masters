...
F1 inferida empiricamente, a semente e os hiperparâmetros não
publicados pelo paper, a contagem das ``22 \emph{features}'', a
substituição do SVM no \emph{combined} e a cobertura reduzida do
\emph{sweep} combinado, encerrando com o resumo das limitações.
...
Tabelas 10--13, e que qualquer comparação \emph{ours vs.\ paper}
deve ser lida como ``ordem de grandeza'' e não como ``match exato''.
...
\begin{itemize}[leftmargin=*,itemsep=0pt]
  \item \textbf{Com \texttt{clean\_df}}: as variantes são
        ...
        predição majoritária --- nossos números ficam muito
...
\subsection{Métrica F1 inferida empiricamente}

Como o paper não especifica a chamada exata de \texttt{sklearn}, a
métrica adotada (\texttt{weighted}) é inferência empírica
(\S{}\ref{sec:protocolo_metrica}). Se evidência adicional surgir
(por exemplo, código do paper sendo publicado) reservamos o
direito de re-rodar com a métrica correta. Nesse cenário, todos os
...
\begin{itemize}[leftmargin=*,itemsep=0pt]
  ...
  \item Métrica F1 inferida empiricamente (não declarada no paper).
  ...
  \item Hiperparâmetros não-cabeçalho dos \emph{baselines} deixados
        ...
        entre versões.
  \item Pipeline \emph{paper-faithful} sem \texttt{clean\_df} ---
