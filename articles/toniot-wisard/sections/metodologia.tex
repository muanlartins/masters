...
todos os modelos; em seguida descrevemos o pipeline de
pré-processamento, as extensões de termômetros na \texttt{wisardpkg} e o
ambiente de \emph{hardware} e reprodutibilidade.

\subsection{Protocolo \emph{paper-faithful}}
\label{sec:protocolo}

Para que a comparação entre os \emph{baselines} clássicos do paper e
a família WiSARD seja uma comparação direta sob protocolo idêntico, todos os experimentos seguem o
mesmo protocolo, derivado da Seção VI--A do paper Alsaedi 2020
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.27\linewidth} p{0.32\linewidth} p{0.32\linewidth}}
    ...
    Métrica F1 & F-score (\S{}VI-B1, não especifica \emph{average}) & \texttt{f1\_weighted} (inferência empírica --- ver \S{}\ref{sec:protocolo_metrica}) \\
...
ou \texttt{'weighted'}) muda os valores reportados em até 0{,}2 pontos em
algumas bases, sendo decisiva para qualquer reprodução. O texto do paper
\emph{resolve} essa ambiguidade na discussão da Tabela 13, onde declara
calcular ``\emph{a weighted average\ldots\ multiplying each class with a
weight factor (i.e., the \# instances with that target class)}'' --- que
é exatamente o \texttt{average='weighted'} do \texttt{sklearn}. Nossa
varredura empírica das três variantes contra as Tabelas 10--11
(\emph{per-device}) confirma essa leitura
(Tabela~\ref{tab:f1_metric}):
...
\subsection{Hardware e reprodutibilidade}
