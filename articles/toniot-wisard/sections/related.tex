...
Esta seção cobre três frentes. A primeira apresenta o dataset TON\_IoT e
o paper de Alsaedi 2020 que reproduzimos, com seu protocolo e suas três
tarefas. A segunda revisa a literatura de \emph{leak} e qualidade do
TON\_IoT, mostrando que as críticas existentes cobrem a camada de rede,
mas não a telemetria que auditamos. A terceira descreve a família WiSARD
de redes neurais sem peso e os cinco modelos avaliados.
...
\subsection{Leak de dados e qualidade do TON\_IoT}
\label{sec:related_leak}

A comunidade já documentou \emph{leak} (\emph{data leakage}: quando
uma \emph{feature} carrega informação do rótulo que não estaria
...
investigar, sem qualquer análise de \emph{leak}. A crítica de
...
\emph{feature} categórica, e nenhum conecta o determinismo da
associação à sub-amostragem da classe \texttt{normal} nos arquivos
públicos. É exatamente este o foco da nossa auditoria
(Seção~\ref{sec:dataset}): diagnosticamos um \emph{leak} categórico
determinístico (Cramér's V $=1{,}000$, associação categórica máxima
em que a \emph{feature} determina o rótulo) em \texttt{Fridge} e
\texttt{Garage\_Door} e o rastreamos até a sub-amostragem
$35\,\text{k} \rightarrow 15\,\text{k}$ da classe \texttt{normal}.
Esse mecanismo é distinto do \emph{leak} de identificadores da camada
de rede e, até onde alcançamos na literatura, ainda não foi reportado
na telemetria.
...
entrada. A versão seminal, a \textbf{WiSARD}~\cite{aleksander1984wisard},
...
\begin{description}[leftmargin=*,itemsep=0pt]
  ...
  \item[\textbf{ULEEN}~\cite{susskind2023uleen}.] Adiciona uma fase
        ...
        de \emph{bleaching} treinável (limiar sobre as contagens de
        acesso das RAMs que descarta respostas de baixa frequência),
        alcançando F1 superior ao BTHOWeN em datasets de visão ao
        custo de mais iterações.
\end{description}
...
\medskip
\noindent A literatura de qualidade do TON\_IoT trata apenas da camada
de rede, e a literatura de telemetria aceita a acurácia quase-perfeita
sem auditá-la; já os trabalhos sobre a família WiSARD não a avaliam sob
um protocolo de reprodução fiel a um \emph{baseline} estabelecido. Este
relatório ocupa essa lacuna dupla: audita o TON\_IoT no nível da
\emph{feature} categórica e avalia cinco modelos WiSARD sob o protocolo
\emph{paper-faithful} de Alsaedi 2020.
